...
PIX_OUTPUT pix(in VERT_OUTPUT_BEAM input) : SV_TARGET
{
	...
	float endT = max(0.001, min((1 - input.uv.x), input.fadeAlpha));
	...
	float4 halfHighlightColor = float4(0.5, 0.219, 0.213, 1);
	float4 baseColor = float4(1, 0, 0, 1);
	float4 color2 = float4(0.24, 0, 0.47, 1);
	
	float noiseBaseExp = pow(saturate(noiseBase * 1.5), 1);
	float4 baseNoiseColor = lerp(baseColor, color2 * noiseBaseExp, noiseBaseExp);
	
	float4 layer1 = halfHighlightColor * innerCombined * innerSharp; //tinting the processed noise and masking it with innerSharp
	float4 col = lerp(baseNoiseColor, layer1, innerGrad); //blending in red using a cylinder gradient
	col = lerp(baseColor * noiseBase, col, innerBeam); //breaking up the outline
