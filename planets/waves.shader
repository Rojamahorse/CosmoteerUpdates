...
Texture2D _normalsTexture2;
SamplerState _normalsTexture2_SS;
float _waveT1;
float _waveT2;
...
PIX_OUTPUT pix(in VERT_OUTPUT input) : SV_TARGET
{
    ...
    float3 tsNrml1 = colorToNormals(_normalsTexture.Sample(_normalsTexture_SS, uv).rgb * input.color.rgb);
    float3 tsNrml2 = colorToNormals(_normalsTexture2.Sample(_normalsTexture2_SS, uv).rgb * input.color.rgb);
    float3 tsNrml = tsNrml1 * _waveT1 + tsNrml2 * _waveT2;
    tsNrml.z /= _waveT1 + _waveT2;
