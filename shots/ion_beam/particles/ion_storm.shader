...
//	float4 unpackedNormal = float4(finalNormal, baseAlpha);
	...
	float colShift = saturate(sin(input.uv.x * 0.01 * macroScale) * sin(input.uv.y * 0.025 * macroScale)) * colShiftIntensity;
	col = lerp(col, _color3.rgb, colShift);

	return float4(col, unpackedNormal.a * input.color.a * fadeAlpha);
}