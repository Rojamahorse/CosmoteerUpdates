...
PIX_OUTPUT pix(in VERT_OUTPUT_NEBULA input) : SV_TARGET
{
	...
	float2 scrolling2 = float2(scrolling.x, -scrolling.y);

	float vTex1 = 1 - _noiseTex2.Sample(_noiseTex2_SS, input.worldLoc * 0.002 + scrolling2).r;
	float vTex2 = 1 - _noiseTex2.Sample(_noiseTex2_SS, input.worldLoc * 0.000235 - scrolling).r;
	float vTex3 = 1 - _noiseTex2.Sample(_noiseTex2_SS, -input.worldLoc * 0.000056 + scrolling2).r;
	float vTex4 = 1 - _noiseTex2.Sample(_noiseTex2_SS, -input.worldLoc * 0.00041 - scrolling).r;

	float3 dir = float3(rotate(float2(0, 0.2), vTex1 * TWO_PI), 0.8);
	float mask = 1 - saturate(dot(dir, unpackedNormal.xyz));
	float edgeMask = pow(saturate(mask * 2 * ((1 - vTex2) + (1 - vTex4))), 3);
	float topMask = pow((1 - mask) * vTex1 * vTex2 * vTex3 * vTex4, 4);

	float3 col4sub = float3(0.02, 0.6, 0.35);
	col += _color5.rgb * saturate(topMask * 20);
